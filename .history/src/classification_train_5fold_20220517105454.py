...
for train_index,test_index in kf.split(data):
    ...
    #train_pll,val_pll=[pll[i] for i in train_index],[pll[j] for j in test_index]
    ...
    #covid.append(("1",wt_seq))
    #val_seq=val_seq+covid
    #for i in range(6):
    #    val_labels.append(np.zeros((9,2)))
    #    #val_pll.append(val_pll[i])
    train_idx, train_strs, train_tokens = batch_converter(train_seq)
    val_idx, val_strs, val_tokens = batch_converter(val_seq)
    # val_onehot_binary=seqs_to_binary_onehot(val_strs,wt_seq)
    # train_onehot_binary=seqs_to_binary_onehot(train_strs,wt_seq)
    # train_dataset=DMS_dataset_pl((train_onehot_binary,train_tokens),train_labels)
    # val_dataset=DMS_dataset_pl((val_onehot_binary,val_tokens),val_labels)

    #train_pppl=[[compute_pppl(s,esm_model, alphabet)] for s in train_strs]
    #val_pppl=[[compute_pppl(s,esm_model, alphabet)] for s in val_strs]
    #train_dataset=DMS_dataset_pl((train_pppl,train_tokens),train_labels)
    #val_dataset=DMS_dataset_pl((val_pppl,val_tokens),val_labels)
    ...
    # #val_pll=torch.tensor(val_pll,device=device)
    # for batch_token in torch.split(val_tokens,10):
    #     #print("batch:",batch_token.device)
    #     #batch_token=batch_token.cuda()
    #     #batch_pll=batch_pll.cuda()
    #     predict=model(batch_token,labels=None)
    #     all_result=np.append(all_result,predict.logits.cpu().detach().numpy(),axis=0)
    # with open("/home/chenn0a/chenn0a/covid_esm1b/esm_GCN_class/17_accu_30/prediction_val_"+str(args.fold)+".npy","ab") as f1 :
    #     np.save(f1,all_result)
    # voc_idx, voc_strs, voc_tokens = batch_converter(covid)
    # voc_predict=model(voc_tokens,labels=None)
    # with open("/home/chenn0a/chenn0a/covid_esm1b/esm_GCN_class/17_accu_30/prediction_voc_val_"+str(args.fold)+".npy","ab") as f2 :
    #     np.save(f2,voc_predict.logits.cpu().detach().numpy())
    f=f+1
    break
