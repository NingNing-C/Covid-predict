...
class DMS_data(torch.utils.data.Dataset):
    def __init__(self,
        ...
        with open(seq_path, "r") as f:  
            seq = f.read() 
        self.seq=seq
        # def get_seq(row,sequence):
        #     _, idx, mt = row[0], int(row[1:-1])-1, row[-1]
        #     sequence = sequence[:idx] + mt + sequence[(idx + 1) :]
        #     return(sequence)
        dms_data = pd.read_csv(data_path,index_col=0)
        if dms_data.isnull().values.any() !=False:
            ValueError("There is nan in the data.")
        # dms_data["seq"]=dms_data.apply(
        # lambda row : get_seq(row["mutation_RBD"],seq),axis=1
        # )
        self.dms_data=dms_data
        #self.regre_label=dms_data.iloc[:,5:14].to_numpy()
...
class DMS_dataset_pl(torch.utils.data.Dataset):
    def __init__(self, encodings, class_label):
        #self.sites=np.array(encodings[0])
        #self.onehot=encodings[0]
        #self.pppl=encodings[0]
        self.encodings = encodings
        #self.regre_label = regre_label
        self.class_label = class_label

    def __getitem__(self, idx):
        item = {"input_ids":self.encodings[idx]}
        item["labels"] = (torch.tensor(self.class_label[idx]))
        #item["onehot"] = (self.onehot[idx])
        #item["pppl"] = (self.pppl[idx])
        #item["site"] = (self.sites[idx])
        #item['regre_label'] = torch.tensor(self.regre_label[idx])
        #item['class_label'] = torch.tensor(self.class_label[idx])
        return item
