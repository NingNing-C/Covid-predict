...
import torch
import datasets
from EsmModel.class_model import covid_prediction_model
import esm
import numpy as np
import torch.nn.functional as F
import time
import os
from OpenAttack.attack_assist.substitute.word import ProteinBlosum62Substitute
import pandas as pd
...
# input_file='/home/chenn0a/chenn0a/covid_esm1b/mHER/mHER_data_as_paper_r3/init_seq/prediction_0.txt'
# success_file_out='/home/chenn0a/chenn0a/covid_esm1b/mHER_attack_result/success_attack.txt'
# failed_file_out='/home/chenn0a/chenn0a/covid_esm1b/mHER_attack_result/failed_attack.txt'

#wt_seq="WGGDGFYAMD"
#mHER_neg=pd.read_csv('/home/chenn0a/chenn0a/covid_esm1b/AttackProtein/src/data/mHER_H3_AgNeg_rmdup.csv',index_col=0)

# init_seq=[]
# with open(input_file,'r') as f:
#     for line in f:
#         init_seq.append(line.strip('\n').split("\t")[0])
#init_seq=init_seq[0:3]
...
class tokenizer:
    def __init__(self,esm_path) :
        self.alphabet = esm.pretrained.load_model_and_alphabet_local(esm_path)[1]
...
class ProteinClassifier(oa.Classifier):
    def __init__(self,esm_path,model_path,cutoff):
        self.model=covid_prediction_model(esm_path=esm_path,freeze_bert=True)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model.load_state_dict(torch.load(model_path,map_location=self.device))
        self.model.to(self.device)
        self.model.eval()
        self.tokenizer=tokenizer(esm_path)
        ...
        # self.esm_model,alphabet=esm.pretrained.load_model_and_alphabet_local(esm_path)
        # self.batch_converter=PLLBatchConverter(alphabet)
        # self.mask_idx = torch.tensor(alphabet.mask_idx)
        # self.esm_model.eval()
        # if torch.cuda.is_available() :
        #     self.esm_model = self.esm_model.cuda()
        #     self.mask_idx = self.mask_idx.cuda()
        # self.wt="WGGDGFYAMD"
        #_,_,self.wt_toks,_=self.batch_converter([("WT", self.wt, -1)])
    ...
    def get_prob(self,input_):
        ...
        for batch_token in torch.split(token,10):
            ...
            #print("batch_token:",batch_token.device)
...
def main():
    ...
    init_seq_path="/home/chenn0a/chenn0a/covid_esm1b/attack_result/covid/init_seq_gisaid/"+date+"_"+str(cutoff)+".csv"
    if os.path.exists(init_seq_path):
        ...
        init_seq.to_csv('/home/chenn0a/chenn0a/covid_esm1b/attack_result/covid/init_seq_gisaid/'+date+"_"+str(cutoff)+".csv")
    ...
    victim = ProteinClassifier(esm_path,model_path,cutoff=cutoff)
